...
struct PanelFooter: View {
            ...
            ZStack {
                HStack(spacing: 10) {
                    ...
                    if !activeStyleCycled {
                        ...
                        .accessibilityLabel("提示：按住 Command 点击可切换显示")
                    }
    ...
    private var chip: some View {
        let label: String = {
            ...
            case .usage: return superAgentStore.range.displayName
            case .cost:  return superAgentStore.range.displayName
    ...
    private var activeLoading: Bool {
        switch screenPref.screen {
        case .usage, .cost: return superAgentStore.loading
        case .credits: return creditsStore.loading
        }
    }

    private var activeLastUpdated: Date? {
        switch screenPref.screen {
        case .usage, .cost: return superAgentStore.lastUpdated
        case .credits: return creditsStore.lastUpdated
        }
    }
    ...
    private var liveStatus: some View {
        ...
        .accessibilityHint("点击立即刷新")
        .accessibilityAddTraits(.isButton)
    }

    private func triggerRefresh() {
        switch screenPref.screen {
        case .usage, .cost: superAgentStore.refresh()
        case .credits: creditsStore.refresh()
        }
    }

    private var liveStatusSpoken: String {
        if activeLoading { return "同步中" }
        if let updated = activeLastUpdated { return "已同步 \(relative(updated))" }
        return "待同步"
    }
